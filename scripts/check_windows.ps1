...
function Test-Command {
    param([Parameter(Mandatory = $true)][string]$Name,[Parameter(Mandatory = $true)][string]$InstallHint)
...
if (Get-Command python -ErrorAction SilentlyContinue) {
    ...
    if ($pythonSupported -ne "yes") { Write-Host "[invalid] Python 3.10 or newer is required." -ForegroundColor Red; $allPresent = $false }
}
...
} catch { Write-Host "[invalid] PopEx cannot write to $dataDir" -ForegroundColor Red; $allPresent = $false }

$venvPython = Join-Path $repoRoot ".venv\Scripts\python.exe"
if (Test-Path $venvPython) {
    & $venvPython -c "import numpy, scipy, librosa, soundfile; print('[ok]      local audio-analysis Python packages')"
