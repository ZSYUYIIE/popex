# Linux CPU profile third-party inventory

This inventory covers exactly the index-fetched distributions in `requirements.lock` and `torch.lock`. The local `popex-demucs-worker` package is PopEx source under the repository license, not a third-party package. Release packaging must still preserve the exact license files shipped with every selected wheel.
...
| numpy | 2.5.1 | BSD-3-Clause plus bundled notices | PyPI | Required by Demucs runtime source |
...
| click | 8.4.2 | BSD-3-Clause | PyPI | Hugging Face CLI/runtime dependency |
...
| hf-xet | 1.6.0 | Apache-2.0 | PyPI | Hugging Face content transport dependency |
...
| openunmix | 1.3.0 | Not installed | Not a Demucs 4.1.0 runtime dependency and its inference path introduces torchaudio. |
| dora-search | 0.1.12 | Not installed | Demucs training-only dependency. |
| Demucs training extra | 4.1.0 `[train]` | Not installed | Training stack is outside the local separation runtime. |
| NVIDIA/CUDA packages | None | Not installed | This is a CPU-only profile. |

## Models

No model weights, model URLs, readiness manifests, or Hugging Face cache assets are included in this profile. The audited model is prepared only through a later explicit worker action.
