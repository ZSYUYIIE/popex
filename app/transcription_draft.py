...
def validate_transcription_draft(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Return a deterministic validated copy of an editable schema-1 draft."""
    ...
    evidence = _interpretation_evidence(
    ...
    nested_ids = _nested_alternative_ids(
        [*parts, *voices, *measures, *phrases, *pitched, *percussion]
    )
    _global_ids(
        parts,
        voices,
...
def write_transcription_draft(
    job_id: str,
    settings: Settings,
    payload: Mapping[str, Any],
) -> Path:
    """Validate and atomically publish an editable interpretation draft."""
    ...
    try:
        ...
        if _directory_snapshot(directory, job_dir) != snapshot:
            raise TranscriptionDraftError(
                "Interpretation draft directory changed during publication."
            )
        ...
        raise TranscriptionDraftError(
            "Interpretation draft could not be published safely."
        ) from exc
    finally:
...
def load_transcription_draft(
    job_id: str,
    settings: Settings,
) -> dict[str, Any] | None:
    """Load and revalidate the published editable interpretation draft."""
    job_dir = _secure_job_root(job_id, settings)
    ...
    try:
        ...
        raise TranscriptionDraftError(
            "Saved interpretation draft is unavailable."
        ) from exc
    ...
    try:
        ...
        raise TranscriptionDraftError(
            "Saved interpretation draft is unreadable or corrupted."
        ) from exc
    try:
        return validate_transcription_draft(payload)
    except TranscriptionDraftValidationError as exc:
        raise TranscriptionDraftError(
            "Saved interpretation draft failed schema validation."
        ) from exc
...
def _source(
    ...
    if _integer(
        source["schemaVersion"], "sourceTranscription.schemaVersion"
    ) != 1:
        raise TranscriptionDraftValidationError(
            "sourceTranscription.schemaVersion must be 1."
        )
    provenance = _mapping(source["provenance"], "sourceTranscription.provenance")
    if not provenance:
        raise TranscriptionDraftValidationError(
            "sourceTranscription.provenance must not be empty."
        )
    events = _sequence(
        source["sourceEventIndex"], "sourceTranscription.sourceEventIndex"
    )
    ...
    return {
        ...
        "provenance": _safe_mapping(
            provenance, "sourceTranscription.provenance"
        ),
...
def _source_event(value: Any, index: int) -> dict[str, Any]:
    ...
    event_type = _choice(
        event["eventType"], {"pitched", "percussion"}, f"{label}.eventType"
    )
    start = _number(
        event["rawStartSeconds"], f"{label}.rawStartSeconds", minimum=0
    )
    end = _number(
        event["rawEndSeconds"], f"{label}.rawEndSeconds", minimum=0
    )
    if end < start or (event_type == "pitched" and end == start):
        raise TranscriptionDraftValidationError(
            f"{label} has an invalid raw range."
        )
    output = {
...
def _algorithms(value: Any) -> dict[str, Any]:
    ...
    for name in sorted(records):
        ...
        if "version" not in record:
            raise TranscriptionDraftValidationError(
                f"algorithms.{name}.version is required."
            )
        clean = {
            "version": _version(
                record["version"], f"algorithms.{name}.version"
            )
        }
...
def _interpretation_evidence(
    ...
    evidence = _mapping(value, "interpretationEvidence")
    _keys(evidence, set(_EVIDENCE_KEYS), set(), "interpretationEvidence")
    output: dict[str, Any] = {}
    for name in _EVIDENCE_KEYS:
        record = _mapping(evidence[name], f"interpretationEvidence.{name}")
        _validate_evidence_shape(name, record)
        if name not in algorithms:
            raise TranscriptionDraftValidationError(
                f"algorithms.{name} is required for interpretation evidence."
            )
        version = _version(
            record["version"], f"interpretationEvidence.{name}.version"
        )
        if algorithms[name]["version"] != version:
            raise TranscriptionDraftValidationError(
                f"interpretationEvidence.{name}.version must match algorithms.{name}.version."
            )
        copied = _evidence_value(
            record,
            f"interpretationEvidence.{name}",
            depth=0,
        )
        assert isinstance(copied, dict)
        expected_type = (
            "pitched"
            if name == "pitchedPartInference"
            else "percussion"
            if name == "percussionInterpretation"
            else None
        )
...
def _validate_evidence_shape(name: str, record: Mapping[str, Any]) -> None:
    if name == "pitchedPartInference":
        required = {
            "version",
            "parts",
        ...
        _keys_open(record, required, f"interpretationEvidence.{name}")
        _require_alias(
            record,
            ("unassignedEventIds", "unassigned_event_ids"),
            f"interpretationEvidence.{name}",
        )
        return
    if name == "percussionInterpretation":
        ...
        _require_alias(
            record,
            ("unresolvedEventIds", "unresolved_event_ids"),
            f"interpretationEvidence.{name}",
        )
        return
    required_aliases = (
        ("meterCandidates", "meter_candidates"),
        ("eventInterpretations", "event_interpretations"),
        ("restCandidates", "rest_candidates"),
    )
    _keys_open(
        record,
        {"version", "measures", "warnings", "diagnostics"},
        f"interpretationEvidence.{name}",
    )
    for aliases in required_aliases:
        _require_alias(record, aliases, f"interpretationEvidence.{name}")


def _require_alias(
    value: Mapping[str, Any], aliases: tuple[str, str], label: str
) -> None:
    present = [name for name in aliases if name in value]
    if len(present) != 1:
        raise TranscriptionDraftValidationError(
            f"{label} must contain exactly one of {aliases[0]} or {aliases[1]}."
        )


def _evidence_value(value: Any, label: str, *, depth: int) -> Any:
    if depth > _MAX_EVIDENCE_DEPTH:
        raise TranscriptionDraftValidationError(f"{label} is nested too deeply.")
    if value is None or isinstance(value, bool):
        return value
    if isinstance(value, str):
        return _text(value, label, _MAX_TEXT, allow_empty=True)
    if isinstance(value, int):
        return value
    if isinstance(value, float):
        if not math.isfinite(value):
            raise TranscriptionDraftValidationError(f"{label} must be finite.")
        return value
    if isinstance(value, Mapping):
        if len(value) > _MAX_EVIDENCE_KEYS:
            raise TranscriptionDraftValidationError(
                f"{label} contains too many fields."
            )
        output: dict[str, Any] = {}
        for key in sorted(value):
            _metadata_key(key, label)
            output[key] = _evidence_value(
                value[key], f"{label}.{key}", depth=depth + 1
            )
        return output
    if isinstance(value, Sequence) and not isinstance(
        ...
        if len(value) > _MAX_COLLECTION:
            raise TranscriptionDraftValidationError(
                f"{label} contains too many values."
            )
        return [
            _evidence_value(item, f"{label}[{index}]", depth=depth + 1)
            for index, item in enumerate(value)
        ]
    raise TranscriptionDraftValidationError(
        f"{label} is not safe JSON evidence."
    )
    ...
    *,
    ...
    if isinstance(value, Mapping):
        for key, child in value.items():
            ...
            if normalized in _RAW_REFERENCE_SCALARS:
            ...
            elif normalized in _RAW_REFERENCE_LISTS:
                if not isinstance(child, list):
                    raise TranscriptionDraftValidationError(
                        f"{child_label} must be an array of raw event IDs."
                    )
                for index, event_id in enumerate(child):
                    _validate_evidence_reference(
                        event_id,
                        source,
                        f"{child_label}[{index}]",
...
def _validate_evidence_reference(
    ...
    *,
    ...
    if not isinstance(value, str) or value not in source:
        raise TranscriptionDraftValidationError(
            f"{label} references an unknown source raw event."
        )
    if expected_type is not None and source[value]["eventType"] != expected_type:
        raise TranscriptionDraftValidationError(
            f"{label} references the wrong source raw event type."
        )
...
def _parts(
    ...
    for index, raw in enumerate(items):
        ...
        clean = {
            ...
            "sourceKind": _slug(
                item["sourceKind"], f"{label}.sourceKind"
            ),
            "role": _slug(item["role"], f"{label}.role"),
            "instrumentKind": _slug(
                item["instrumentKind"], f"{label}.instrumentKind"
            ),
            "voiceIds": _ids(item["voiceIds"], f"{label}.voiceIds"),
            "sourceEventIds": _source_refs(
                item["sourceEventIds"], source, f"{label}.sourceEventIds"
            ),
            "confidence": _confidence(
                item["confidence"], f"{label}.confidence"
            ),
        }
...
def _voices(
    ...
    items = _bounded_sequence(value, "voices", _MAX_COLLECTION)
    output = []
    for index, raw in enumerate(items):
        label = f"voices[{index}]"
        item = _mapping(raw, label)
        required = {
            "id",
            "partId",
        ...
        clean = {
            ...
            "partId": part_id,
            "voiceKind": _slug(
                item["voiceKind"], f"{label}.voiceKind"
            ),
            "sourceEventIds": _source_refs(
                item["sourceEventIds"], source, f"{label}.sourceEventIds"
            ),
...
def _measures(value: Any) -> list[dict[str, Any]]:
    items = _bounded_sequence(value, "measures", _MAX_COLLECTION)
    output = []
    for index, raw in enumerate(items):
        ...
        required = {
            ...
            "rawStartSeconds",
            "rawEndSeconds",
            "interpretedStartSeconds",
            "interpretedDurationSeconds",
            "confidence",
        }
        _keys_open(item, required, label)
        raw_start, raw_end = _range(
            item,
            "rawStartSeconds",
            "rawEndSeconds",
            label,
        ...
        interpreted_start = _number(
            item["interpretedStartSeconds"],
            f"{label}.interpretedStartSeconds",
            minimum=0,
        )
        duration = _number(
            item["interpretedDurationSeconds"],
            f"{label}.interpretedDurationSeconds",
            minimum=0,
            positive=True,
        )
        clean = {
            "id": _id(item["id"], f"{label}.id"),
            "index": _integer_range(
                item["index"], f"{label}.index", 0, _MAX_INDEX
            ),
            "rawStartSeconds": raw_start,
        ...
        if (
            "startBeatIndex" in clean
            and clean["endBeatIndex"] <= clean["startBeatIndex"]
        ):
            raise TranscriptionDraftValidationError(
                f"{label} has invalid beat bounds."
            )
        _extras(
            item,
            clean,
        ...
        output.append(clean)
    _unique(output, "measure")
    output.sort(key=lambda item: (item["index"], item["id"]))
    for expected, item in enumerate(output):
        if item["index"] != expected:
            raise TranscriptionDraftValidationError(
                "Measure indices must be contiguous from zero."
            )
        if expected:
            previous = output[expected - 1]
            if item["interpretedStartSeconds"] < (
                previous["interpretedStartSeconds"]
                + previous["interpretedDurationSeconds"]
            ):
                raise TranscriptionDraftValidationError(
                    "Measures must not overlap."
                )
    return output
...
def _phrases(
    ...
    source: Mapping[str, Any],
    parts: Mapping[str, Any],
    voices: Mapping[str, Any],
    measures: Mapping[str, Any],
) -> list[dict[str, Any]]:
    items = _bounded_sequence(value, "phrases", _MAX_COLLECTION)
    output = []
    for index, raw in enumerate(items):
        ...
        if voices[voice_id]["partId"] != part_id:
            raise TranscriptionDraftValidationError(
                f"{label}.voiceId does not belong to partId."
            )
        refs = _source_refs(
            item["sourceEventIds"],
            source,
        ...
        if "measureIds" in item:
            clean["measureIds"] = _refs(
                item["measureIds"], measures, f"{label}.measureIds"
            )
        _extras(
            item,
            clean,
    ...
    return sorted(
        output, key=lambda item: (item["rawStartSeconds"], item["id"])
    )
...
def _pitched_items(
    ...
    voices: Mapping[str, Any],
    measures: Mapping[str, Any],
    phrases: Mapping[str, Any],
) -> list[dict[str, Any]]:
    items = _bounded_sequence(value, "pitchedItems", _MAX_COLLECTION)
    output = []
    for index, raw in enumerate(items):
        label = f"pitchedItems[{index}]"
        item = _mapping(raw, label)
        required = {
            "id",
            "interpretationType",
        ...
        kind = _choice(
            ...
            {"note", "rest", "unassigned"},
            f"{label}.interpretationType",
        )
        clean = _item_common(
            item,
            label,
        ...
        if kind == "note" and "pitch" not in item:
            raise TranscriptionDraftValidationError(
                f"{label}.pitch is required for note hypotheses."
            )
        if kind == "rest" and (
            "pitch" in item or "tieCandidate" in item
        ):
            raise TranscriptionDraftValidationError(
                f"{label} rest hypotheses cannot contain pitch or tie data."
            )
        if "pitch" in item:
            clean["pitch"] = _pitch(item["pitch"], f"{label}.pitch")
        if "tieCandidate" in item:
            clean["tieCandidate"] = _tie(
                item["tieCandidate"], f"{label}.tieCandidate"
            )
        _extras(
            item,
            clean,
...
def _percussion_items(
    ...
    items = _bounded_sequence(value, "percussionItems", _MAX_COLLECTION)
    output = []
    for index, raw in enumerate(items):
        label = f"percussionItems[{index}]"
        item = _mapping(raw, label)
        required = {
            "id",
            "placementStatus",
        ...
        if any(
            voices[voice_id]["partId"] != clean["partId"]
            for voice_id in voice_ids
        ):
            raise TranscriptionDraftValidationError(
                f"{label}.voiceIds must belong to partId."
            )
        hits = _bounded_sequence(item["hits"], f"{label}.hits", 64)
        if not hits:
            raise TranscriptionDraftValidationError(
                f"{label}.hits must not be empty."
            )
        normalized_hits = [
            _hit(hit, hit_index, label, voices, clean["partId"])
            for hit_index, hit in enumerate(hits)
        ]
        normalized_hits.sort(key=lambda hit: hit["sourceHitIndex"])
        if len(
            {hit["sourceHitIndex"] for hit in normalized_hits}
        ) != len(normalized_hits):
            raise TranscriptionDraftValidationError(
                f"{label}.hits have duplicate sourceHitIndex values."
            )
        hit_voice_ids = sorted(
            {
                hit["voiceId"]
        ...
        if sorted(voice_ids) != hit_voice_ids:
            raise TranscriptionDraftValidationError(
                f"{label}.voiceIds must match hit voice assignments."
            )
        clean["voiceIds"] = voice_ids
        clean["hits"] = normalized_hits
        _extras(
            item,
            clean,
            required
...
def _item_common(
    ...
    *,
    ...
    status_value = _choice(
        ...
        {"placed", "unassigned"},
        f"{label}.placementStatus",
    )
    clean: dict[str, Any] = {
        "id": _id(item["id"], f"{label}.id"),
        "placementStatus": status_value,
        "partId": part_id,
        "sourceEventIds": refs,
        "rawStartSeconds": start,
        "rawEndSeconds": end,
    ...
    if "measureId" in item:
        clean["measureId"] = _ref(
            item["measureId"], measures, f"{label}.measureId"
        )
    if "phraseId" in item:
        clean["phraseId"] = _ref(
            item["phraseId"], phrases, f"{label}.phraseId"
        )
    _interpreted_pair(
        item,
        clean,
        label,
        required=status_value == "placed",
    )
    if status_value == "placed" and "gridPosition" not in item:
        raise TranscriptionDraftValidationError(
            f"{label}.gridPosition is required when placed."
        )
    if "gridPosition" in item:
        clean["gridPosition"] = _grid(
            item["gridPosition"],
            f"{label}.gridPosition",
            measures,
            start,
        )
        ...
        if "measureId" in clean and grid_measure != clean["measureId"]:
            raise TranscriptionDraftValidationError(
                f"{label}.gridPosition.measureId must match measureId."
            )
    if "sharedEvidence" in item:
        clean["sharedEvidence"] = _boolean(
            item["sharedEvidence"], f"{label}.sharedEvidence"
        )
    return clean
...
def _grid(
    ...
    if "beatIndex" in grid:
        output["beatIndex"] = _integer_range(
            grid["beatIndex"], f"{label}.beatIndex", 0, _MAX_INDEX
        )
    has_measure = bool(
        {"measureId", "measureIndex", "beatInMeasure"}.intersection(grid)
    )
    if has_measure:
        if not {
            "measureId",
            ...
            raise TranscriptionDraftValidationError(
                f"{label} has incomplete measure placement."
            )
        measure_id = _ref(
            grid["measureId"], measures, f"{label}.measureId"
        )
        measure_index = _integer_range(
            grid["measureIndex"],
            f"{label}.measureIndex",
            0,
            _MAX_INDEX,
        )
        if measures[measure_id]["index"] != measure_index:
            raise TranscriptionDraftValidationError(
                f"{label}.measureIndex does not match measureId."
            )
        output.update(
            measureId=measure_id,
            measureIndex=measure_index,
            beatInMeasure=_integer_range(
                grid["beatInMeasure"],
                f"{label}.beatInMeasure",
                1,
                _MAX_INDEX,
            ),
        )
    has_subdivision = bool(
        {"subdivision", "subdivisionIndex"}.intersection(grid)
    )
    if has_subdivision:
        if not {"subdivision", "subdivisionIndex"} <= set(grid):
            raise TranscriptionDraftValidationError(
                f"{label} has incomplete subdivision placement."
            )
        subdivision = _subdivision(
            grid["subdivision"], f"{label}.subdivision"
        )
        subdivision_index = _integer_range(
            grid["subdivisionIndex"],
    ...
    has_time = bool(
        {"alignedTimeSeconds", "offsetSeconds"}.intersection(grid)
    )
    if has_time:
        if not {"alignedTimeSeconds", "offsetSeconds"} <= set(grid):
            raise TranscriptionDraftValidationError(
                f"{label} has incomplete aligned timing."
            )
        aligned = _number(
            grid["alignedTimeSeconds"],
        ...
        if not math.isclose(
            ...
            raise TranscriptionDraftValidationError(
                f"{label}.offsetSeconds must equal raw minus aligned time."
            )
        output.update(alignedTimeSeconds=aligned, offsetSeconds=offset)
    if not output:
        raise TranscriptionDraftValidationError(
            f"{label} must contain placement evidence."
        )
    return output


def _pitch(value: Any, label: str) -> dict[str, Any]:
    pitch = _mapping(value, label)
    allowed = {"midiNote", "midiPitch", "frequencyHz", "noteName"}
    if not set(pitch) or set(pitch) - allowed:
        raise TranscriptionDraftValidationError(
            f"{label} has invalid fields."
        )
    output: dict[str, Any] = {}
    if "midiNote" in pitch:
        output["midiNote"] = _integer_range(
            pitch["midiNote"], f"{label}.midiNote", 0, 127
        )
    if "midiPitch" in pitch:
        output["midiPitch"] = _number(
            pitch["midiPitch"], f"{label}.midiPitch"
        )
    if "frequencyHz" in pitch:
        output["frequencyHz"] = _number(
            pitch["frequencyHz"],
            f"{label}.frequencyHz",
            minimum=0,
            positive=True,
        )
    if "noteName" in pitch:
        output["noteName"] = _text(
            pitch["noteName"], f"{label}.noteName", 64
        )
    return output


def _tie(value: Any, label: str) -> dict[str, Any]:
    tie = _mapping(value, label)
    required = {"role", "confidence"}
    _keys_open(tie, required, label)
    output = {
        "role": _slug(tie["role"], f"{label}.role"),
        "confidence": _confidence(
            tie["confidence"], f"{label}.confidence"
        ),
    }
    if "targetItemId" in tie:
        output["targetItemId"] = _id(
            tie["targetItemId"], f"{label}.targetItemId"
        )
...
def _hit(
    ...
    label = f"{parent}.hits[{index}]"
    hit = _mapping(value, label)
    required = {"sourceHitIndex", "rawKind", "broadVoice", "confidence"}
    _keys_open(hit, required, label)
    output = {
        "sourceHitIndex": _integer_range(
            hit["sourceHitIndex"],
            f"{label}.sourceHitIndex",
            0,
            _MAX_INDEX,
        ),
        "rawKind": _slug(hit["rawKind"], f"{label}.rawKind"),
        "broadVoice": _slug(
            hit["broadVoice"], f"{label}.broadVoice"
        ),
        "confidence": _confidence(
            hit["confidence"], f"{label}.confidence"
        ),
    }
    if "voiceId" in hit:
...
def _top_alternatives(
    ...
    items = _bounded_sequence(value, "alternatives", _MAX_COLLECTION)
    output = []
    subjects = {
        "part": parts,
        "voice": voices,
        "measure": measures,
        "phrase": phrases,
        "pitched_item": pitched,
        "percussion_item": percussion,
    }
    for index, raw in enumerate(items):
        ...
        subject_type = _choice(
            item["subjectType"], set(subjects), f"{label}.subjectType"
        )
        clean = {
            "id": _id(item["id"], f"{label}.id"),
        ...
        if "sourceEventIds" in item:
            clean["sourceEventIds"] = _source_refs(
                item["sourceEventIds"],
                source,
                f"{label}.sourceEventIds",
            )
        _extras(item, clean, required | {"sourceEventIds"}, label)
        output.append(clean)
...
def _extras(
    ...
    for key in sorted(source):
        if key in known:
            continue
        if key == "warnings":
            output[key] = _warnings(
                source[key], f"{label}.warnings", _MAX_WARNINGS
            )
        elif key == "alternatives":
            output[key] = _nested_alternatives(
                source[key], f"{label}.alternatives"
            )
        else:
            _metadata_key(key, label)
            output[key] = _safe_value(
                source[key], f"{label}.{key}", 1
            )


def _nested_alternatives(value: Any, label: str) -> list[dict[str, Any]]:
    items = _bounded_sequence(value, label, _MAX_ALTERNATIVES)
    output = []
    for index, raw in enumerate(items):
        item_label = f"{label}[{index}]"
        item = _mapping(raw, item_label)
        required = {"id", "kind", "confidence"}
        _keys_open(item, required, item_label)
        clean = {
            "id": _id(item["id"], f"{item_label}.id"),
            "kind": _slug(item["kind"], f"{item_label}.kind"),
            "confidence": _confidence(
                item["confidence"], f"{item_label}.confidence"
            ),
        }
        _extras(item, clean, required, item_label)
        output.append(clean)
...
def _safe_mapping(value: Mapping[str, Any], label: str) -> dict[str, Any]:
    ...
    for key in sorted(value):
        _metadata_key(key, label)
        output[key] = _safe_value(value[key], f"{label}.{key}", 1)
    return output
...
def _safe_value(value: Any, label: str, depth: int) -> Any:
    ...
    if isinstance(value, Sequence) and not isinstance(
        ...
        return [
            _safe_value(item, f"{label}[{index}]", depth + 1)
            for index, item in enumerate(value)
        ]
    raise TranscriptionDraftValidationError(
        f"{label} is not safe JSON metadata."
    )


def _part_voice_lists(
    parts: Sequence[Mapping[str, Any]], voices: Mapping[str, Any]
) -> None:
    for part in parts:
        for voice_id in part["voiceIds"]:
            if (
                voice_id not in voices
                or voices[voice_id]["partId"] != part["id"]
            ):
                raise TranscriptionDraftValidationError(
                    "Part voiceIds must reference voices belonging to the part."
                )


def _primary_assignments(
    pitched: Sequence[Mapping[str, Any]],
    percussion: Sequence[Mapping[str, Any]],
) -> None:
    owners: dict[str, list[Mapping[str, Any]]] = {}
    for item in [*pitched, *percussion]:
        for event_id in item["sourceEventIds"]:
            owners.setdefault(event_id, []).append(item)
    for event_id, items in owners.items():
        if len(items) > 1 and not all(
            item.get("sharedEvidence") is True for item in items
        ):
            raise TranscriptionDraftValidationError(
                f"Source event {event_id!r} is assigned more than once."
            )


def _tie_targets(
    items: Sequence[Mapping[str, Any]], index: Mapping[str, Any]
) -> None:
    for item in items:
        tie = item.get("tieCandidate")
        if not tie or "targetItemId" not in tie:
            continue
        target = tie["targetItemId"]
        if (
            target not in index
            or target == item["id"]
            or index[target]["interpretationType"] == "rest"
        ):
            raise TranscriptionDraftValidationError("Invalid tie target.")


def _nested_alternative_ids(
...
def _global_ids(*collections: Any) -> None:
    ...
    for collection in collections:
        if collection and isinstance(
            collection[0] if isinstance(collection, list) else None, str
        ):
            ids.extend(collection)
        else:
            ids.extend(item["id"] for item in collection)
    if len(ids) != len(set(ids)):
        raise TranscriptionDraftValidationError(
            "Draft IDs must be globally unique."
        )
...
def _unique(items: Sequence[Mapping[str, Any]], label: str) -> None:
    ...
    if len(ids) != len(set(ids)):
        raise TranscriptionDraftValidationError(f"Duplicate {label} ID.")
...
def _source_envelope(
    ...
    if start != expected_start or end != expected_end:
        raise TranscriptionDraftValidationError(
            f"{label} raw range must retain its source evidence envelope."
        )


def _interpreted_pair(
    item: Mapping[str, Any],
    output: dict[str, Any],
    label: str,
    *,
    ...
    if present and len(present) != 2:
        raise TranscriptionDraftValidationError(
            f"{label} has incomplete interpreted timing."
        )
    if required and len(present) != 2:
        raise TranscriptionDraftValidationError(
            f"{label} requires interpreted timing when placed."
        )
    if present:
        output["interpretedStartSeconds"] = _number(
            item["interpretedStartSeconds"],
            f"{label}.interpretedStartSeconds",
            minimum=0,
        )
        output["interpretedDurationSeconds"] = _number(
            item["interpretedDurationSeconds"],
            f"{label}.interpretedDurationSeconds",
            minimum=0,
        )
...
def _paired_ints(
    ...
    *,
    ...
    if present and len(present) != 2:
        raise TranscriptionDraftValidationError(
            f"{label}.{first} and {second} must appear together."
        )
    if present:
        output[first] = _integer_range(
            item[first], f"{label}.{first}", minimum, _MAX_INDEX
        )
        output[second] = _integer_range(
            item[second], f"{label}.{second}", minimum, _MAX_INDEX
        )
...
def _range(
    ...
    *,
    ...
    start = _number(item[first], f"{label}.{first}", minimum=0)
    end = _number(item[second], f"{label}.{second}", minimum=0)
...
def _source_refs(
    ...
    *,
    ...
    if expected_type and any(
        source[event_id]["eventType"] != expected_type for event_id in refs
    ):
        raise TranscriptionDraftValidationError(
            f"{label} references the wrong source event type."
        )
    return sorted(
        refs,
        key=lambda event_id: (source[event_id]["rawStartSeconds"], event_id),
    )
...
def _ref(value: Any, index: Mapping[str, Any], label: str) -> str:
    ...
    if ref not in index:
        raise TranscriptionDraftValidationError(
            f"{label} references a missing object."
        )
    return ref
...
def _ids(value: Any, label: str) -> list[str]:
    ...
    if len(items) > _MAX_LIST:
        raise TranscriptionDraftValidationError(
            f"{label} contains too many IDs."
        )
    output = [_id(item, f"{label}[{index}]") for index, item in enumerate(items)]
...
def _warnings(value: Any, label: str, maximum: int) -> list[str]:
    ...
    if len(items) > maximum:
        raise TranscriptionDraftValidationError(
            f"{label} contains too many warnings."
        )
    return [
        _text(item, f"{label}[{index}]", _MAX_WARNING)
...
def _metadata_key(value: Any, label: str) -> str:
    if not isinstance(value, str) or not _KEY.fullmatch(value):
        raise TranscriptionDraftValidationError(
            f"{label} has an unsafe field name."
        )
    if re.sub(r"[^a-z0-9]", "", value.lower()) in _FORBIDDEN:
        raise TranscriptionDraftValidationError(
            f"{label} contains prohibited data."
        )
    return value
...
def _text(
    ...
    *,
    ...
    if not isinstance(value, str):
        raise TranscriptionDraftValidationError(f"{label} must be a string.")
    if (
        value != value.strip()
        or (not value and not allow_empty)
        or len(value) > maximum
    ):
        raise TranscriptionDraftValidationError(f"{label} has invalid text.")
    if any(ord(character) < 32 and character not in "\t\n\r" for character in value):
        raise TranscriptionDraftValidationError(
            f"{label} contains control characters."
        )
    _unsafe_path_text(value, label)
    return value
...
def _unsafe_path_text(value: str, label: str) -> None:
    ...
    for candidate in (value, decoded):
        if "\x00" in candidate:
            raise TranscriptionDraftValidationError(f"{label} contains NUL.")
        ...
        if (
            ...
            or _DRIVE.match(candidate)
            or _URI.match(candidate)
            or candidate.lower().startswith("file:")
            or ".." in parts
            or _MACHINE.search(normalized)
        ):
            raise TranscriptionDraftValidationError(
                f"{label} contains an unsafe path or URI."
            )
...
def _utc(value: Any, label: str) -> str:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() != timedelta(0):
        raise TranscriptionDraftValidationError(f"{label} must be UTC.")
    return text
...
def _number(
    ...
    *,
    ...
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise TranscriptionDraftValidationError(f"{label} must be a number.")
    number = float(value)
    ...
    if positive and number <= 0:
        raise TranscriptionDraftValidationError(f"{label} must be positive.")
...
def _integer_range(value: Any, label: str, minimum: int, maximum: int) -> int:
    ...
    if not minimum <= number <= maximum:
        raise TranscriptionDraftValidationError(f"{label} is out of range.")
    return number
...
def _subdivision(value: Any, label: str) -> int | str:
    ...
    if isinstance(value, int) and value > 0:
        return value
    if isinstance(value, str) and _SUBDIVISION.fullmatch(value):
        return value
    raise TranscriptionDraftValidationError(f"{label} is invalid.")
...
def _sequence(value: Any, label: str) -> list[Any]:
    if not isinstance(value, Sequence) or isinstance(
        ...
        raise TranscriptionDraftValidationError(f"{label} must be an array.")
    return list(value)
...
def _bounded_sequence(value: Any, label: str, maximum: int) -> list[Any]:
    items = _sequence(value, label)
    if len(items) > maximum:
        raise TranscriptionDraftValidationError(f"{label} is too large.")
    return items
...
def _encoded(payload: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise TranscriptionDraftValidationError(
            "Draft is not valid JSON data."
        ) from exc
    if len(data) > _MAX_ARTIFACT_BYTES:
        raise TranscriptionDraftValidationError(
            "Interpretation draft is too large."
        )
    return data
...
def _secure_job_root(job_id: str, settings: Settings) -> Path:
    if not isinstance(job_id, str) or not _JOB.fullmatch(job_id):
        raise TranscriptionDraftValidationError("Invalid job identifier.")
    try:
        exports = settings.exports_dir
        info = exports.lstat()
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise TranscriptionDraftError(
                "Interpretation draft job directory is unsafe."
            )
        root = exports.resolve(strict=True)
        lexical = exports / job_id
        job_info = lexical.lstat()
        ...
        if (
            job_root.parent != root
            or secure_job_dir(settings, job_id).resolve(strict=True) != job_root
        ):
            raise TranscriptionDraftError(
                "Interpretation draft job directory is unsafe."
            )
        return job_root
    except (TranscriptionDraftError, TranscriptionDraftValidationError):
        raise
    except (AttributeError, MediaProcessingError, OSError, RuntimeError) as exc:
        raise TranscriptionDraftError(
            "Interpretation draft job directory is unavailable."
        ) from exc
...
def _artifact_directory(job_dir: Path, *, create: bool) -> Path | None:
    ...
    if create:
        try:
            ...
            raise TranscriptionDraftError(
                "Interpretation draft directory could not be created safely."
            ) from exc
    try:
        directory.lstat()
    except FileNotFoundError:
        ...
        raise TranscriptionDraftError(
            "Interpretation draft directory is unavailable."
        ) from exc
...
def _directory_snapshot(
    ...
    try:
        ...
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            raise TranscriptionDraftError(
                "Interpretation draft directory is unsafe."
            )
...
def _existing_destination(path: Path, directory: Path) -> None:
    try:
        ...
        raise TranscriptionDraftError(
            "Existing interpretation draft is unavailable."
        ) from exc
...
def _write_file(path: Path, data: bytes, directory: Path) -> None:
    ...
    try:
        ...
        if not stat.S_ISREG(os.fstat(fd).st_mode):
            raise TranscriptionDraftError(
                "Temporary interpretation draft is unsafe."
            )
    except TranscriptionDraftError:
        raise
    except OSError as exc:
        raise TranscriptionDraftError(
            "Temporary interpretation draft could not be written."
        ) from exc
    finally:
        if fd is not None:
            os.close(fd)
...
def _replace_atomic(source: Path, destination: Path) -> None:
    try:
        ...
        raise TranscriptionDraftError(
            "Interpretation draft could not replace the published artifact."
        ) from exc
...
def _regular_file(path: Path, directory: Path) -> os.stat_result:
    try:
        ...
        if (
            ...
            raise TranscriptionDraftError("Interpretation draft path is unsafe.")
        return info
    except TranscriptionDraftError:
        raise
    except OSError as exc:
        raise TranscriptionDraftError(
            "Interpretation draft path is unsafe."
        ) from exc


def _read_file(path: Path, directory: Path) -> bytes:
    before = _regular_file(path, directory)
    ...
    try:
        ...
        if _snapshot(opened) != _snapshot(before):
            raise TranscriptionDraftError(
                "Saved interpretation draft changed during validation."
            )
        chunks: list[bytes] = []
        total = 0
        while True:
            ...
            if total > _MAX_ARTIFACT_BYTES:
                raise TranscriptionDraftError(
                    "Saved interpretation draft is too large."
                )
        ...
        raise TranscriptionDraftError(
            "Saved interpretation draft could not be read safely."
        ) from exc
    finally:
        if fd is not None:
            os.close(fd)
    ...
    if (
        after_open is None
        or _snapshot(before) != _snapshot(after_open)
