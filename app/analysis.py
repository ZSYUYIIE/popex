...
BASELINE_COLLECTIONS = (
    ("ionian", "major", MAJOR_PROFILE),
    ("aeolian", "minor", MINOR_PROFILE),
)
...
def analyze_audio(
    ...
    if not wav_path.is_file():
        raise AudioAnalysisError(
            "Analysis audio is missing. Prepare analysis.wav before running "
            "audio analysis."
        )

    stage_callback(
        "analyzing_audio",
        "Analyzing audio level and signal quality.",
        66,
    )
    ...
    if duration_seconds <= 0 or duration_seconds > settings.max_duration_seconds + 1:
        raise AudioAnalysisError(
            "Analysis audio has an invalid or unsupported duration."
        )
    ...
    if not math.isfinite(peak) or not math.isfinite(rms):
        raise AudioAnalysisError(
            "Analysis audio contains invalid numerical samples."
        )
    if rms <= settings.audio_silence_rms_threshold:
        raise AudioAnalysisError(
            "Analysis audio is silent or too quiet to analyze reliably."
        )

    warnings: list[str] = []
    if rms <= settings.audio_silence_rms_threshold * 10:
        warnings.append(
            "The signal is very quiet; timing and tonal estimates may be unreliable."
        )
    rms_dbfs = float(
        20.0 * math.log10(max(rms, np.finfo(np.float64).tiny))
    )

    stage_callback(
        "detecting_beats",
        "Analyzing timing and estimating beats.",
        74,
    )
    ...
    stage_callback(
        "estimating_key",
        "Estimating tuning and tonal center.",
        84,
    )
    tonality = _analyze_tonality(mono, sample_rate)
    _check_timeout(started, settings)
    if tonality["symbol"] is None:
        warnings.append("A reliable global tonal center could not be estimated.")
    ...
    stage_callback(
        "saving_analysis",
        "Saving versioned audio analysis.",
        94,
    )
    ...
    try:
        ...
        raise AudioAnalysisError(
            "Audio analysis completed but its JSON result could not be saved."
        ) from exc
    _check_timeout(started, settings)
...
def load_analysis(job_id: str, settings: Settings) -> dict[str, Any] | None:
    ...
    if payload.get("schemaVersion") != 1:
        raise AudioAnalysisError(
            "Saved audio analysis uses an unsupported schema version."
        )
    return payload
...
def _read_wav(path: Path) -> tuple[np.ndarray, int]:
    try:
        ...
        raise AudioAnalysisError(
            "Analysis audio is unreadable or corrupted."
        ) from exc
    if (
        sample_rate <= 0
        or audio.ndim != 2
        ...
        raise AudioAnalysisError(
            "Analysis audio is empty or has unsupported sample data."
        )
...
def _analyze_timing(mono: np.ndarray, sample_rate: int) -> dict[str, Any]:
    ...
    if (
        onset_envelope.size < 2
        or float(np.max(onset_envelope)) <= np.finfo(float).eps
    ):
        return {
            "tempoBpm": None,
    ...
    if beat_frames.size:
        valid_frames = beat_frames[
            (beat_frames >= 0) & (beat_frames < onset_envelope.size)
        ]
        beat_strength = (
            float(np.mean(onset_envelope[valid_frames]))
    ...
    stability_score = (
        max(0.0, 1.0 - min(1.0, (interval_cv or 0.5) / 0.25))
        if intervals.size >= 2
        else 0.0
    )
    beat_confidence = (
        float(
            np.clip(
                0.55 * strength_score + 0.45 * stability_score,
                0.0,
                1.0,
            )
        )
    ...
    meter, meter_confidence, downbeats = _estimate_meter(
        onset_envelope,
        beat_frames,
        beats,
    )
    return {
        ...
        "tempoConfidence": beat_confidence,
...
def _estimate_meter(
    ...
    strengths = np.asarray(
        [
            ...
            if 0 <= frame < onset_envelope.size
            else 0.0
    ...
    for meter in (3, 4):
        phase_means = np.asarray(
            [np.mean(strengths[phase::meter]) for phase in range(meter)]
        )
...
def _analyze_tonality(mono: np.ndarray, sample_rate: int) -> dict[str, Any]:
    ...
    # The current deterministic baseline evaluates only the Ionian/major and
    # Aeolian/minor key-profile families. Collection names remain open strings
    # so later models can add modal, mixed, or local tonal interpretations.
    for collection, legacy_mode, profile in BASELINE_COLLECTIONS:
        normalized_profile = profile / np.linalg.norm(profile)
        for root, name in enumerate(KEY_NAMES):
            rotated = np.roll(normalized_profile, root)
            score = float(
                np.dot(chroma_normalized, rotated)
                / (np.linalg.norm(chroma_normalized) + 1e-12)
            )
            candidates.append(
                {
                    "tonalCenter": name,
                    "collection": collection,
                    "displayName": f"{name} {legacy_mode}",
    ...
    for index, item in enumerate(candidates[:5]):
        candidate_confidence = (
            ...
            if index == 0
                ...
                np.clip(
                    confidence - (best["score"] - item["score"]) / 0.08,
                    0.0,
                    1.0,
                )
            )
    ...
    return {
        ...
        "chromaticismScore": None,
        "baselineCollections": ["ionian", "aeolian"],
        # Compatibility fields retained until clients adopt primaryCandidate.
        "key": best["tonalCenter"],
        "mode": best["legacyMode"],
        "symbol": best["displayName"],
        "confidence": confidence,
        "scoreMargin": margin,
        "tuningOffsetCents": tuning_cents,
        "chromaMean": [
            float(value) for value in chroma_normalized.tolist()
        ],
...
def _validate_json_numbers(value: Any) -> None:
    if isinstance(value, dict):
        ...
        raise AudioAnalysisError(
            "Audio analysis produced an invalid numerical result."
        )
