...
def process_url(job_id: str, source_url: str, settings: Settings, stage_callback: StageCallback, progress_callback: ProgressCallback) -> MediaResult:
    job_dir = secure_job_dir(settings, job_id, create=True)
    stage_callback("extracting_audio", "Downloading and extracting source audio.", 5)
    try:
        import yt_dlp
    ...
    def match_filter(info: Mapping[str, Any], *, incomplete: bool = False) -> str | None:
        ...
        if duration and float(duration) > settings.max_duration_seconds:
            return f"Video duration exceeds the {settings.max_duration_seconds}-second limit"
        return None
    ...
    def progress_hook(data: Mapping[str, Any]) -> None:
        if data.get("status") == "downloading":
            ...
            if total:
                progress_callback(min(35.0, max(8.0, downloaded / total * 27 + 8)))
    ...
    options: dict[str, Any] = {
        ...
        "quiet": True,
        "no_warnings": True,
        "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "mp3", "preferredquality": settings.audio_quality}],
    }
    try:
    ...
    if not source_path.is_file():
        candidates = sorted(path for path in job_dir.iterdir() if path.is_file() and path.name != "analysis.wav")
        if not candidates:
            cleanup_job_dir(job_dir)
            raise MediaProcessingError("Extraction completed but no source audio file was produced.")
        source_path = candidates[0]
    title = str(info.get("title") or "Imported URL audio")
    uploader = info.get("uploader") or info.get("channel")
    return prepare_source(
        job_id=job_id, source_path=source_path, title=title,
        uploader=str(uploader) if uploader else None, settings=settings,
        stage_callback=stage_callback, progress_callback=progress_callback,
        metadata_extra={
            "source_type": "url", "source_url": source_url,
            "extractor": info.get("extractor_key") or info.get("extractor"),
            "source_id": info.get("id"), "webpage_url": info.get("webpage_url") or source_url,
        },
    )


def process_upload(job_id: str, source_file_name: str, original_filename: str, settings: Settings, stage_callback: StageCallback, progress_callback: ProgressCallback) -> MediaResult:
    ...
    return prepare_source(
        job_id=job_id, source_path=source_path, title=Path(original_filename).stem or "Uploaded media",
        uploader=None, settings=settings, stage_callback=stage_callback, progress_callback=progress_callback,
        metadata_extra={"source_type": "upload", "original_filename": original_filename},
    )


def prepare_source(*, job_id: str, source_path: Path, title: str, uploader: str | None, settings: Settings, stage_callback: StageCallback, progress_callback: ProgressCallback, metadata_extra: Mapping[str, Any]) -> MediaResult:
    job_dir = secure_job_dir(settings, job_id)
    source_path = resolve_job_file(job_dir, source_path.name)
    stage_callback("validating", "Inspecting source media with ffprobe.", 40)
    ...
    if duration is not None and duration > settings.max_duration_seconds:
        raise MediaProcessingError(f"Media duration exceeds the {settings.max_duration_seconds}-second limit.")

    analysis_path = job_dir / "analysis.wav"
    stage_callback("normalizing", "Creating 44.1 kHz PCM analysis audio.", 48)
    with _processing_lock:
    ...
    metadata = {
        "job_id": job_id, "title": title, "uploader": uploader, "duration_seconds": duration,
        ...
        "source_file": source_path.name, "normalized_wav": analysis_path.name,
    ...
    return MediaResult(
        title=title, uploader=uploader, duration_seconds=duration,
        source_format=source_metadata["format_name"], sample_rate=source_metadata["sample_rate"],
        channel_count=source_metadata["channel_count"], source_file_name=source_path.name,
...
def probe_media(path: Path, settings: Settings) -> dict[str, Any]:
    ...
    command = [settings.ffprobe_binary, "-v", "error", "-show_entries", "format=duration,format_name:stream=codec_type,sample_rate,channels", "-of", "json", str(path)]
    ...
    return {
        ...
        "channel_count": int(audio_stream["channels"]) if audio_stream.get("channels") else None,
    }
...
def normalize_audio(source_path: Path, output_path: Path, settings: Settings) -> None:
    ...
    command = [settings.ffmpeg_binary, "-hide_banner", "-loglevel", "error", "-y", "-i", str(source_path), "-map", "0:a:0", "-vn", "-c:a", "pcm_s16le", "-ar", "44100", str(output_path)]
    run_command(command, settings=settings, paths=(source_path, output_path))
    if not output_path.is_file() or output_path.stat().st_size == 0:
        output_path.unlink(missing_ok=True)
        raise MediaProcessingError("FFmpeg did not produce the analysis WAV.")


def run_command(command: list[str], *, settings: Settings, paths: tuple[Path, ...] = ()) -> subprocess.CompletedProcess[str]:
    try:
        return subprocess.run(command, check=True, capture_output=True, text=True, timeout=max(120, settings.max_duration_seconds * 2))
    except FileNotFoundError as exc:
        raise MediaProcessingError(f"Required executable '{command[0]}' is not available on PATH.") from exc
        ...
        raise MediaProcessingError(friendly_error(detail, settings=settings, paths=paths)) from exc


def require_binary(binary: str, display_name: str) -> None:
    if Path(binary).is_file() or shutil.which(binary):
        return
    raise MediaProcessingError(f"{display_name} is unavailable. Install it and ensure '{binary}' is on PATH.")
...
def friendly_error(error: Exception | str, *, settings: Settings, paths: tuple[Path, ...] = ()) -> str:
